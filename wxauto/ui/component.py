...
from wxauto.uia import ControlFromHandle, RollIntoView, ListItemControl
...
class SelectContactWnd(BaseUISubWnd):
    ...
    def __init__(self, parent):
        ...
        if hwnd:
            ...
            self.control = parent.root.control.PaneControl(
                ClassName=self._ui_cls_name, searchDepth=1
            )
    ...
    def send(self, target, message=None, exact=True):
        if isinstance(target, str):
            ...
            checkboxs = [
                i
                for i in checkboxlist.GetChildren()
                if i.ControlTypeName == "CheckBoxControl"
            ]
            if checkboxs:
                if exact and (
                    exact_checkboxs := [i for i in checkboxs if i.Name == target]
                ):
...
class AlertDialog(BaseUISubWnd):
    ...
    def __init__(self, parent):
        self.parent = parent
        self.root = parent.root
        self.control = parent.root.control.WindowControl(
            ClassName=self._ui_cls_name, searchDepth=1
        )

    def get_all_text(self):
        return [
            i.Name for i in self.control.FindAll(control_type="TextControl") if i.Name
        ]
...
class ProfileWnd(BaseUISubWnd):
    ...
    def add_friend(
        ...
        while time.time() - t0 < 5:
            if NewFriendsWnd.exists(0.1):
                ...
                i for i in self.root._get_windows() if i.Name == "添加朋友请求"
            ]:
    ...
    def modify_remark_tags(self, remark: str = None, tags: list = None):
        ...
        if remark:
            edit = (
                dialogwnd.TextControl(Name=self._lang("备注名"))
                .GetParentControl()
                .EditControl()
            )
            edit.Click()
        ...
        if tags:
            edit_btn = (
                dialogwnd.TextControl(Name=self._lang("标签"))
                .GetParentControl()
                .ButtonControl()
            )
            edit_btn.Click()
...
class AddFriendWindow(BaseUISubWnd):
    ...
    def _lang(self, text: str) -> str:
        return ADD_NEW_FRIEND_WINDOW.get(text, {WxParam.LANGUAGE: text}).get(
            WxParam.LANGUAGE
        )
    ...
    def add(
        ...
        tipscontrol = self.control.TextControl(
            Name=self._lang("你的联系人较多，添加新的朋友时需选择权限")
        )

        permission_sns = self.control.CheckBoxControl(
            Name=self._lang("聊天、朋友圈、微信运动等")
        )
        permission_chat = self.control.CheckBoxControl(Name=self._lang("仅聊天"))
        if tipscontrol.Exists(0.5):
            permission_sns = (
                tipscontrol.GetParentControl()
                .GetParentControl()
                .TextControl(Name=self._lang("朋友圈"))
            )
            permission_chat = (
                tipscontrol.GetParentControl()
                .GetParentControl()
                .TextControl(Name=self._lang("仅聊天"))
            )

        if addmsg:
            msgedit = (
                self.control.TextControl(Name=self._lang("发送添加朋友申请"))
                .GetParentControl()
                .EditControl()
            )
            msgedit.DoubleClick()
        ...
        if remark:
            remarkedit = (
                self.control.TextControl(Name=self._lang("备注名"))
                .GetParentControl()
                .EditControl()
            )
            msgedit.DoubleClick()
        ...
        if tags:
            tagedit = (
                self.control.TextControl(Name=self._lang("标签"))
                .GetParentControl()
                .EditControl()
            )
            for tag in tags:
...
class WeChatImage(BaseUISubWnd):
    ...
    def __init__(self) -> None:
        ...
        if self.hwnd:
            ...
            self.t_qrcode = self.ToolsBox.ButtonControl(
                Name=self._lang("识别图中二维码")
            )
    ...
    def save(self, dir_path=None, timeout=10) -> Path:
        ...
        while True:
            ...
            try:
                ...
                if (
                    suffix in image_sufix and not is_valid_image(path)
                ) or not os.path.getsize(path):
                    wxlog.debug("图片格式不正确，删除文件")
                    os.remove(path)
                    continue
                wxlog.debug(
                    f"读取到图片/视频路径[{os.path.exists(path)}, {os.path.getsize(path)}]：{path}"
                )
                break
            except:
                if n > 3:
...
class NoteWindow(BaseUISubWnd):
    ...
    def __init__(self, parent=None, timeout=3):
        self.parent = parent
        self.control = find_window_from_root(
            classname=self._ui_cls_name, timeout=timeout
        )
        self.dialog_texts = ""
    ...
    def init(self, wait=0):
        ...
        if self.control and self.control.Exists(0):
            ...
            if (dialog := self.control.PaneControl(ClassName="WeUIDialog")).Exists(0):
                self.dialog_texts = "".join(
                    [i.Name for i in dialog.FindAll(control_type="TextControl")]
                )
    ...
    def save_all(self, dir_path: Path) -> WxResponse[List[Path]]:
        ...
        if "15" in self.metadata:
            ...
            paths = [
                Path(p)
                for p in re.findall('filepath="(.*?)"', pathdata)
                if os.path.exists(p)
            ]
...
class AddMemberWnd(BaseUISubWnd):
    ...
    def _lang(self, text: str) -> str:
        return ADD_GROUP_MEMBER_WINDOW.get(text, {WxParam.LANGUAGE: text}).get(
            WxParam.LANGUAGE
        )
    ...
    def selected_members_count(self) -> int:
        selected = self.control.TableControl(
            Name=self._lang("已选择联系人"), searchDepth=3
        ).GetChildren()
        return len(selected)
...
class NewFriendElement:
    def __init__(self, control, parent):
        ...
        self.msg = (
            self.control.GetFirstChildControl()
            .PaneControl(SearchDepth=1)
            .GetChildren()[-1]
            .TextControl()
            .Name
        )
        self.NewFriendsBox = self.root._chat_api.control.ListControl(
            Name="新的朋友"
        ).GetParentControl()
    ...
    def __repr__(self) -> str:
        return (
            f"<wxauto New Friends Element at {hex(id(self))} ({self.name}: {self.msg})>"
        )
    ...
    def accept(self, remark=None, tags=None, permission="朋友圈"):
        ...
        tipscontrol = NewFriendsWnd.TextControl(
            Name="你的联系人较多，添加新的朋友时需选择权限"
        )
        ...
        if tipscontrol.Exists(0.5):
            permission_sns = (
                tipscontrol.GetParentControl()
                .GetParentControl()
                .TextControl(Name="朋友圈")
            )
            permission_chat = (
                tipscontrol.GetParentControl()
                .GetParentControl()
                .TextControl(Name="仅聊天")
            )

        if remark:
            remarkedit = (
                NewFriendsWnd.TextControl(Name="备注名")
                .GetParentControl()
                .EditControl()
            )
            remarkedit.Click()
        ...
        if tags:
            tagedit = (
                NewFriendsWnd.TextControl(Name="标签").GetParentControl().EditControl()
            )
            for tag in tags:
                tagedit.Click()
                tagedit.Input(tag)
                NewFriendsWnd.PaneControl(
                    ClassName="DropdownWindow"
                ).TextControl().Click()
    ...
    def get_account(self, wait=5):
        ...
        tags = ["微信号：", "WeChat ID: "]
        for tag in tags:
            if (account_tag_control := self.root.ChatBox.TextControl(Name=tag)).Exists(
                wait
            ):
...
class WeChatBrowser:
    ...
    class _BrowserMenu:
        ...
        def option_names(self):
            return [
                i.Name
                for i in self.control.GetChildren()
    ...
    def get_tabs(self) -> List[_PageTab]:
        tabitem = self.control.PaneControl(searchDepth=1, ClassName="").TabItemControl()
        if tabitem.Exists(0):
    ...
    def search(self, url):
        search_btn_eles = [
            i
            for i in self.control.TabControl().GetChildren()
        ...
        while True:
            ...
            if search_btn_eles:
                ...
                edit = self.control.TabControl().EditControl(
                    Name=self._lang("地址和搜索栏")
                )
    ...
    def forward(self, friend, message, timeout=10):
        ...
        while True:
            ...
            more_control = self.control.PaneControl(
                searchDepth=1, ClassName=""
            ).MenuItemControl(Name=self._lang("更多"))
            more_control.Click()
            time.sleep(0.5)
            copyurl = self.control.PaneControl(
    ...
    def close(self):
        ...
        try:
            self.control.PaneControl(searchDepth=1, ClassName="").ButtonControl(
                Name=self._lang("关闭")
            ).Click()
        except Exception as e:
    ...
    def select_option(self, option: str):
        ...
        menu_control = self.control.PaneControl(
            ClassName="Chrome_WidgetWin_0"
        ).MenuControl(searchDepth=5)
    ...
    def get_url(self, timeout: int = 5) -> str:
        ...
        while (
            ...
            or self.control.DocumentControl()
            .TextControl(Name="mp appmsg sec open")
            .Exists(0)
        ):
            if time.time() - t0 > timeout:
                break
...
def get_wx_browser(func, timeout=10) -> WeChatBrowser:
    ...
    while True:
        ...
        wins = [
            ...
            if i[1] == WeChatBrowser._ui_cls_name and i[-1] == WeChatBrowser._ui_name
        ]
        ...
        while True:
            ...
            for item in listitems:
                ...
                if msgid not in msgids:
                    ...
                    msgtime = parse_wechat_time(
                        item.GetProgenyControl(4, 1, control_type="TextControl").Name
                    )
                    if "[图片]" in item.Name:
                        # wait for image loading
                        for _ in range(10):
                            imgcontrol = item.GetProgenyControl(
                                6, control_type="ButtonControl", refresh=True
                            )
                            if imgcontrol:
                        ...
                        videocontrol = item.GetProgenyControl(
                            5, control_type="ButtonControl", refresh=True
                        )
                        if videocontrol:
            ...
            if (
                ...
                and listitemids
                == [item.GetRuntimeId() for item in listcontrol.GetChildren()]
            ):
                self.control.SendKeys("{Esc}")
                return msgs
...
class ContactManagerWindow(BaseUISubWnd):
    ...
    def __init__(self, parent: BaseUIWnd = None):
        self.parent = parent
        wins = find_all_windows_from_root(
            self._ui_cls_name, self._ui_name, self.parent.pid
        )
        if wins:
            self.control = wins[0]
            self.Sidebar, _, self.ContactBox = self.control.PaneControl(
                ClassName="", searchDepth=3, foundIndex=3
            ).GetChildren()
        else:
    ...
    def get_all_friends(self, speed: int = 5):
        ...
        while n < 5:
            for _, ele in enumerate(contact_ele_list):
                contacts_info = {
                    "nickname": ele.TextControl()
                    .Name.replace("</em>", "")
                    .replace("<em>", ""),
                    "remark": ele.ButtonControl(foundIndex=2)
                    .Name.replace("</em>", "")
                    .replace("<em>", ""),
                    "tags": ele.ButtonControl(foundIndex=3)
                    .Name.replace("</em>", "")
                    .replace("<em>", "")
                    .split("，"),
                }
            ...
            while n < 5:
                nowlist = [
                    i.GetRuntimeId()
                    for i in self.ContactBox.ListControl().GetChildren()
                ]
                if (
                    lastid != nowlist[-1]
                    and lastid in nowlist
                    and top_ele == ele.BoundingRectangle.top
                ):
                    break
    ...
    def get_all_recent_groups(self, speed: int = 1, wait=0.05):
        ...
        group_list_control = (
            self.control.PaneControl(Name="最近群聊").GetParentControl().ListControl()
        )
        groups = []
        ...
        while n < 5:
            for _, item in enumerate(group_list_items):
                text_control1, text_control2 = (
                    item.TextControl().GetParentControl().GetChildren()
                )
            ...
            while n < 5:
                ...
                if (
                    lastid != nowlist[-1]
                    and lastid in nowlist
                    and top_ele == item.BoundingRectangle.top
                ):
                    break
