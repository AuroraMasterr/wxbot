...
from wxauto.ui.component import CMenuWnd
from wxauto.param import (
    WxParam,
    WxResponse,
)
from wxauto.languages import *
from wxauto.utils import SetClipboardText, SetClipboardFiles, uilock
...
class SessionBox:
    ...
    def _lang(self, text: str) -> str:
        return WECHAT_SESSION_BOX.get(text, {WxParam.LANGUAGE: text}).get(
            WxParam.LANGUAGE
        )

    def init(self):
        self.searchbox = self.control.EditControl(Name=self._lang("搜索"))
        self.session_list = self.control.ListControl(
            Name=self._lang("会话"), searchDepth=7
        )
        self.archived_session_list = self.control.ListControl(
            Name=self._lang("折叠的群聊"), searchDepth=7
        )
    ...
    def get_session(self) -> List[SessionElement]:
        if self.session_list.Exists(0):
            ...
            return [
                SessionElement(i, self)
                for i in self.archived_session_list.GetChildren()
            ]
        else:
            return []
    ...
    def search(
        self, keywords: str, force: bool = False, force_wait: Union[float, int] = 0.5
    ):
        ...
        search_result = self.control.ListControl(
            RegexName=".*?IDS_FAV_SEARCH_RESULT.*?"
        )
        ...
        while time.time() - t0 < WxParam.SEARCH_CHAT_TIMEOUT:
            ...
            for search_result_item in search_result_items:
                ...
                if (
                    ...
                    and search_result_item.TextControl(
                        Name=f"微信号: <em>{keywords}</em>"
                    ).Exists(0)
                ):
                    ...
                    search_result_item.ControlTypeName == "ListItemControl"
                    and search_result_item.TextControl(
                        Name=f"群聊名称: <em>{highlight_who}</em>"
                    ).Exists(0)
                ):
    ...
    def open_contact_manager(self):
        wxlog.debug("打开联系人管理")
        while not (
            contact_btn := self.control.ButtonControl(Name=self._lang("通讯录管理"))
        ).Exists(0):
            self.go_top()
...
class SessionElement:
    def __init__(self, control: Control, parent: SessionBox, debug_output: bool = True):
        self.root = parent.root
        self.parent = parent
        self.control = control
        info_controls = [
            i
            for i in self.control.GetProgenyControl(3).GetChildren()
            if i.ControlTypeName == "TextControl"
        ]
        ...
        self.content = (
            temp_control.Name
            if (
                temp_control := control.GetProgenyControl(
                    4, -1, control_type="TextControl"
                )
            )
            else None
        )
