...
class WeChatSubWnd(BaseUISubWnd):
    ...
    def __init__(self, key: Union[str, int], parent: "WeChatMainWnd", timeout: int = 3):
        self.root = self
        self.parent = parent
    ...
    def _get_chatbox(self, nickname: str = None, exact: bool = False) -> ChatBox:
    ...
    def send_files(self, filepath, who=None, exact=False) -> WxResponse:
        chatbox = self._get_chatbox(who, exact)
        if chatbox is None:
            return WxResponse.failure(f"未找到聊天窗口：{who}")
    ...
    def send_emotion(
        self, index: int, who: str = None, exact: bool = False
    ) -> WxResponse:
        chatbox = self._get_chatbox(who, exact)
        if chatbox is None:
            return WxResponse.failure(f"未找到聊天窗口：{who}")
    ...
    def get_group_members(self, who: str = None, exact: bool = False) -> List[str]:
        chatbox = self._get_chatbox(who, exact)
        if chatbox is None:
            return []
    ...
    def add_friend_from_group(
        ...
        if chatbox is None:
            return WxResponse.failure(f"未找到聊天窗口：{who}")
        return chatbox.add_friend_from_group(index, addmsg, remark, tags, permission)

    def get_group_member_info(self, index: int, who: str = None, exact: bool = False):
        chatbox = self._get_chatbox(who, exact)
        if chatbox is None:
            return WxResponse.failure(f"未找到聊天窗口：{who}")
...
class WeChatMainWnd(WeChatSubWnd):
    ...
    def __init__(self, nickname: str = None, hwnd: int = None):
        ...
        if hwnd:
            ...
            if not hwnd:
                raise Exception(
                    f"未找到微信窗口：{nickname}，如您是4.0微信客户端，请在官网下载3.9客户端使用本项目：https://pc.weixin.qq.com"
                )
            self._setup_ui(hwnd)
    ...
    def _setup_ui(self, hwnd: int):
        try:
            ...
            navigation_control, sessionbox_control, chatbox_control = (
                MainControl2.GetChildren()
            )
            ...
            raise Exception(
                f"WeChat实例初始化失败，请将该文件发给管理员反馈：{debug_file}"
            )
    ...
    def _get_chatbox(self, nickname: str = None, exact: bool = False) -> ChatBox:
        if nickname and (chatbox := WeChatSubWnd(nickname, self, timeout=0)).control:
            ...
            if nickname:
                switch_result = self._session_api.switch_chat(
                    keywords=nickname, exact=exact
                )
    ...
    def get_sub_wnd(self, who: str, timeout: int = 0):
        if hwnd := FindWindow(
            classname=WeChatSubWnd._ui_cls_name, name=who, timeout=timeout
        ):
    ...
    def get_new_friend_request(
        self, acceptable: bool = False
        ...
        # self.SessionBox.WheelUp(wheelTimes=3)
        cti.Click()
        new_friend_list = self._chat_api.control.ListControl(
            Name=self._lang("新的朋友")
        )
        ...
        NewFriendsList = [
            NewFriendElement(i, self) for i in new_friend_list.GetChildren()
        ]
        if acceptable:
    ...
    def _get_next_new_message(self, filter_mute: bool = False):
        ...
        def get_new_session(filter_mute):
            ...
            if sessions[0].name == self._lang("折叠的群聊"):
                ...
                sessions = self._session_api.get_session()
            new_sessions = [
                i for i in sessions if i.isnew and i.name != self._lang("折叠的群聊")
            ]
            if filter_mute:
    ...
    def send_card(
        self, url: str, friends: str = None, message: str = None, timeout: int = 10
    ) -> WxResponse:
        if not friends:
    ...
    def add_new_friend(
        ...
        try:
            ...
            edit.Click()
            edit.Input(keyword)
            search_result = self.SessionBox.TextControl(
                Name=self._lang("搜索结果") + keyword
            )
            ...
            while True:
                ...
                elif self.SessionBox.ListItemControl(
                    Name=self._lang("找不到相关账号或内容")
                ).Exists(0):
                ...
                elif self.control.PaneControl(
                    ClassName="RectToastWnd", searchDepth=1
                ).Exists(0):
                    return WxResponse.success("对方未设置请求确认，已添加成功")
                elif (
                    not self.SessionBox.ListControl()
                    .GetChildren()[0]
                    .Name.startswith(self._lang("搜索结果"))
                ):
                    msg = (
                        self.SessionBox.ListControl()
                        .GetChildren()[0]
                        .TextControl()
                        .Name
                    )
    ...
    def switch_to_chat_page(self):
        t0 = time.time()
        while not self.SessionBox.ButtonControl(
            Name=self._lang("发起群聊"), searchDepth=3
        ).Exists(0):
            if time.time() - t0 > 5:
    ...
    def switch_to_contact_page(self):
        t0 = time.time()
        while not self.SessionBox.ButtonControl(
            Name=self._lang("添加朋友"), searchDepth=3
        ).Exists(0):
            if time.time() - t0 > 5:
                wxlog.error("切换到聊天页面超时")
                break
    ...
    def get_recent_groups(
        self, speed: int = 1, interval: float = 0.05
        ...
        while True:
            for ele, d in list_control.Walk():
                if next_click:
                    ...
                    return next_click
                if (
                    ele.ControlTypeName == "TextControl"
                    and d == 3
                    and ele.Name in letters
                ):
                    next_click = ele.Name
    ...
    def get_friends_details(
        self, n=None, tag=None, timeout=0xFFFFF, save_head_image=False, save_head_wait=0
    ):
        wxlog.debug("获取好友详情")
        letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ#"
        t0 = time.time()
            ...
            while True:
                ...
                for item in items:
                    ...
                    if item.Name == "" and item.TextControl(Name="群聊").Exists(0):
                        ...
                        continue
                    if start:
                        if item not in listitems and item.Name != "":
