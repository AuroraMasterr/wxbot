...
class ChatBox:
    ...
    def get_info(self):
        ...
        for chat_name_control, depth in walk:
            if isinstance(chat_name_control, uia.TextControl):
                break
        if not isinstance(chat_name_control, uia.TextControl) or depth < 8:
            return {}
        ...
        if chat_name_control_count == 1:
            ...
            try:
                ...
                if second_text.startswith("@"):
                    ...
                    chat_info["group_member_count"] = int(
                        second_text.replace("(", "").replace(")", "")
                    )
                    chat_info["chat_type"] = "group"
                    chat_info["chat_name"] = chat_name_control.Name.replace(
                        second_text, ""
                    )
            except:
                chat_info["chat_type"] = "friend"
    ...
    def send_text(self, content: str):
        ...
        while self.editbox.GetValuePattern().Value:
            if time.time() - t0 > 10:
                return WxResponse.failure(f"Timeout --> {self.who} - {content}")
            if (
                toast := self.root.control.PaneControl(
                    ClassName="ToastWnd", searchDepth=2
                )
            ).Exists(0):
                toast.SendKeys("{Esc}")
    ...
    def send_msg(self, content: str, clear: bool = True, at=None, typing: bool = False):
        ...
        if not content and not at:
            return WxResponse.failure(
                f"`content` and `at` can't be empty at the same time"
            )
    ...
    def send_file(self, file_path):
        ...
        while True:
            if time.time() - t0 > WxParam.SEND_FILE_TIMEOUT:
                return WxResponse.failure(
                    f"SendFiles Timeout --> {self.who} - {file_path}"
                )
    ...
    def load_more(self, interval=0.3):
        ...
        while True:
            if len(self.msgbox.GetChildren()) > msg_len:
                ...
                time.sleep(interval)
                if (
                    self.msgbox.GetChildren()[0].BoundingRectangle.top == loadmore_top
    ...
    def get_msgs(self):
        if self.msgbox.Exists(0):
            return [
                ...
                for msg_control in self.msgbox.GetChildren()
    ...
    def get_new_msgs(self):
        if not self.msgbox.Exists(0):
            return []
        if not self.fzx_init_f:
            self.fzx_init_f = True
            self.fzxinit()
        ...
        if not self._empty and (
            (
                not self.used_msg_ids and now_msg_ids
            )  # 没有使用过的消息id，但当前有消息id
        ...
        last_one_msgid = max(
            ...
            key=self.used_msg_ids.index,
            default=None,
        )
        new1 = [x for x in now_msg_ids if x not in used_msg_ids_set]
        new2 = (
            now_msg_ids[now_msg_ids.index(last_one_msgid) + 1 :]
            if last_one_msgid is not None
            else []
        )
        ...
        return [
            parse_msg(msg_control, self)
            for msg_control in new_controls
    ...
    def _get_tail_after_nth_match(self, msgs, last_msg, n):
        matches = [
            i for i, msg in reversed(list(enumerate(msgs))) if msg.content == last_msg
        ]
        if len(matches) >= n:
            ...
            matches = [
                ...
                if msg.content == content and msg.sender_remark == nickname
            ]
            if len(matches) >= n:
                wxlog.debug(f"匹配到基准消息：<{nickname}> {content}")
            else:
                wxlog.debug(
                    f"未匹配到基准消息，以最后一条消息为基准：{msgs[-1].content}"
                )
                matches = [
                    i
                    for i, msg in reversed(list(enumerate(msgs)))
    ...
    def get_next_new_msgs(self, count=None, last_msg=None):
        ...
        msgs = [
            parse_msg(msg_control, self)
            for msg_control in msg_controls
            if msg_control.ControlTypeName == "ListItemControl"
        ]
        ...
        if index is not None:
            ...
            wxlog.debug(
                f"获取以下是新消息下的所有消息(index: {index}, len: {len(return_msgs)})"
            )
            return return_msgs
    ...
    def add_group_members(
        self, members: Union[str, List[str]], reason: str = None
    ) -> WxResponse:
        roominfoWnd = self._open_chat_more_info()
        if isinstance(roominfoWnd, WxResponse):
            return roominfoWnd
    ...
    def merge_and_forward(self, targets: Union[List[str], str], message: str = None):
        wxlog.debug("转发合并消息")
        option_panel = self.control.ToolBarControl(
            Name=self._lang("多选"), searchDepth=9
        )
    ...
    def get_top_msgs(self) -> List["TopMsg"]:
        top_items = []
        if (controls := self.control.GetProgenyControl(5).GetChildren()) and len(
            controls
        ) > 1:
            ...
            if (
                topmsgwnd := self.root.control.PaneControl(
                    ClassName="ChatRoomTopMsgWnd", searchDepth=3
                )
            ).Exists(0):
                top_items = [
                    TopMsg(self, i) for i in topmsgwnd.ListControl().GetChildren()
                ]
        return top_items
...
class EmotionWnd(BaseUISubWnd):
    ...
    def switch_to_my_favorite(self):
        my_emotion_icon = self.control.CheckBoxControl(
            Name=self._lang("添加的单个表情")
        )
            ...
            if index < len(emotions):
                ...
                while True:
                    ...
                    if fourth.GetRuntimeId() == emotions[idx - 1].GetRuntimeId():
                        ...
                        amount = len(emotions)
                        return next_page(
                            new_index, emotion_list, emotions, last_one, idx, amount
                        )
                    emotion_list.WheelDown()
                    time.sleep(0.005)
...
class ChatRoomDetailWnd(BaseUISubWnd):
    ...
    def __init__(self, parent):
        self.parent = parent
        self.root = parent.root
        self.control = self.root.control.Control(
            ClassName=self._ui_cls_name, searchDepth=1
        )

    def _lang(self, text: str) -> str:
        return CHATROOM_DETAIL_WINDOW.get(text, {WxParam.LANGUAGE: text}).get(
            WxParam.LANGUAGE
        )
    ...
    def _edit(self, key, value):
        ...
        btn = (
            self.control.TextControl(Name=key)
            .GetParentControl()
            .ButtonControl(Name=key)
        )
        if btn.Exists(0):
    ...
    def add_member(
        ...
        self.control.ButtonControl(
            Name=self._lang("添加")
        ).GetParentControl().GetChildren()[0].Click()
    ...
    def edit_remark(self, remark):
        ...
        Args:
            remark (str): 新的群聊备注
        """
        if self.control.TextControl(Name=self._lang("仅群主或管理员可以修改")).Exists(
            0
        ):
    ...
    def edit_group_notice(self, notice: Union[str, List[str]]) -> WxResponse:
        ...
        """
        self.control.TextControl(
            Name=self._lang("群公告")
        ).GetParentControl().ButtonControl(Name=self._lang("点击编辑群公告")).Click()
        announcementwnd = uia.WindowControl(
            ClassName="ChatRoomAnnouncementWnd", searchDepth=1
        )
        if announcementwnd.TextControl(Name=self._lang("仅群主和管理员可编辑")).Exists(
            0
        ):
        ...
        announcementwnd.PaneControl(ClassName="WeUIDialog").ButtonControl(
            Name=self._lang("发布")
        ).Click()
        return WxResponse.success()
    ...
    def get_group_members(self, control=False):
        ...
        members = [
            i
            for i in self.control.ListControl(Name=self._lang("聊天成员")).GetChildren()
        ]
...
class DeleteMemberWnd(BaseUISubWnd):
    def __init__(self, parent) -> None:
        ...
        self.control = self.root.control.WindowControl(
            ClassName="DeleteMemberWnd", searchDepth=3
        )
    ...
    def remove(self, keyword):
        ...
        if len(result) == 1:
            ...
            for item in result:
                if item.TextControl().Exists(0) and (
                    item.TextControl().Name == keyword
                    or item.ButtonControl().Name == keyword
                ):
