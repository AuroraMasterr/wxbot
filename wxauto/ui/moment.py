...
from wxauto.param import WxParam, WxResponse
from wxauto.utils import SetClipboardText, FindWindow
from .component import WeChatImage, ProfileWnd
    ...
    def GetMoments(self, next_page=False, speed1=3, speed2=1):
        ...
        if next_page:
            while True:
                ...
                moments_controls = [
                    i
                    for i in self.SnsBox.ListControl(
                        Name=self._lang("朋友圈")
                    ).GetChildren()
        ...
        moments_controls = [
            i
            for i in self.SnsBox.ListControl(Name=self._lang("朋友圈")).GetChildren()
            if i.ControlTypeName == "ListItemControl"
        ]
        moments = [Moments(i, self) for i in moments_controls]
...
class Moments(BaseUISubWnd):
    ...
    def _parse(self):
        ...
        self.info["sender"] = self.control.GetProgenyControl(
            4, control_type="ButtonControl"
        ).Name
        self.info["content"] = content_control.Name if content_control else ""
        self.info["time"] = (
            self.control.ButtonControl(Name=self._lang("评论"))
            .GetParentControl()
            .TextControl()
            .Name
        )
        ...
        if self.control.ListControl(Name=self._lang("评论")).Exists(0):
            self.info["comments"] = [
                i.Name
                for i in self.control.ListControl(Name=self._lang("评论")).GetChildren()
            ]
        ...
        for i in range(10):
            text_control = self.control.GetProgenyControl(
                5, i, control_type="ButtonControl"
            )
            if not text_control:
                break
    ...
    def _download_pic(self, msgitem, dir_path=None):
        uia.RollIntoView(
            self.parent.control.ListControl(),
            msgitem,
            bias=self.parent.ToolsBox.BoundingRectangle.height() * 2,
        )
        msgitem.Click()
        imgobj = WeChatImage()
    ...
    def Comment(self, text):
        uia.RollIntoView(self.parent.SnsBox, self.cmt)
        self.cmt.Click()
        cmt_btn = self.parent.control.PaneControl(
            ClassName="SnsLikeToastWnd"
        ).ButtonControl(Name=self._lang("评论"))
        cmt_btn.Click()
        edit_control: uia.EditControl = self.parent.control.EditControl(
            Name=self._lang("评论")
        )
        t0 = time.time()
        while True:
