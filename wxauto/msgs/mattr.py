...
from wxauto.utils.tools import parse_wechat_time
...
class TickleMessage(SystemMessage):
    ...
    def __init__(
        ...
        super().__init__(control, parent, sub_control_pointer)
        self.tickle_list = [
            i.Name for i in control.ListItemControl().GetParentControl().GetChildren()
        ]
...
class FriendMessage(HumanMessage):
    ...
    def __init__(
        ...
        if (remark_control := self.control.TextControl()).Exists(
            0
    ...
    def _xbias(self):
        if WxParam.FORCE_MESSAGE_XBIAS:
            return int(
                self.head_control.BoundingRectangle.width() * WxParam.BIAS_MULTIPLE
            )
            ...
            try:
                ...
                if profile_wnd := ProfileWnd(self):
                    result = profile_wnd.add_friend(
                        addmsg=addmsg, remark=remark, tags=tags, permission=permission
                    )
                    return result
            except:
                time.sleep(0.1)
