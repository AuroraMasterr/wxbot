...
from wxauto.param import WxResponse, WxParam, PROJECT_NAME
from wxauto.ui.component import CMenuWnd, SelectContactWnd, ProfileWnd, WeChatDialog
...
class BaseMessage(Message):
    ...
    def _get_hash(self, enable: bool = True):
        if enable:
            ...
            self.structure_data = [
                (i.ControlTypeName, i.ClassName, i.Name) for i in sub_controls
            ]
            rect = self.control.BoundingRectangle
            self.hash_text = f"({rect.height()},{rect.width()})" + ";".join(
                [f"{i[0]}:{i[1]},{i[2]}" for i in self.structure_data]
            )
    ...
    def roll_into_view(self) -> WxResponse:
        """滚动消息至显示窗口"""
        if (
            uia.RollIntoView(self.control.GetParentControl(), self.control, equal=True)
            == "not exist"
        ):
...
class HumanMessage(BaseMessage):
    ...
    def roll_into_view(self) -> WxResponse:
        ...
        if (
            uia.RollIntoView(
                self.control.GetParentControl(), self.head_control, equal=True, bias=10
            )
            == "not exist"
        ):
            return WxResponse.failure("消息目标控件不存在，无法滚动至显示窗口")
    ...
    @uilock
    def quote(
        self, text: str, at: Union[List[str], str] = None, timeout: int = 3
    ) -> WxResponse:
        """引用消息
    ...
    def multi_select(self):
        """多选消息"""
        self.roll_into_view()
        if self.parent.control.ToolBarControl(
            Name=self.parent._lang("多选"), searchDepth=9
        ).Exists(0):
            self.control.Click()
        else:
    ...
    @uilock
    def reply(self, text: str, at: Union[List[str], str] = None) -> WxResponse:
        """回复消息
    ...
    @uilock
    def forward(
        self, targets: Union[List[str], str], message: str = None, timeout: int = 3
    ) -> WxResponse:
        """转发消息
