from wxauto.utils.tools import get_file_dir, parse_wechat_time
...
class QuoteMessage(HumanMessage):
    ...
    def __init__(
        ...
        super().__init__(control, parent, sub_control_pointer)
        self.content, self.quote_content = re.findall(
            self._lang("re_引用消息"), self.content, re.DOTALL
        )[0]
    ...
    def download_quote_image(
        self, dir_path: Union[str, Path] = None, timeout: int = 10
    ) -> Path:
        """下载引用消息中的图片或视频"""
