...
from typing import (
    ...
    Iterable,
    Tuple,
)  # need pip install typing for Python3.4 or lower
from .uiplug import *
...
class _DllClient:
    ...
    def __init__(self):
        ...
        if load:
            ...
            self.dll.BitmapFromWindow.argtypes = (
                ctypes.c_size_t,
                ctypes.c_int,
                ctypes.c_int,
                ctypes.c_int,
            ...
            self.dll.BitmapToFile.argtypes = (
                ctypes.c_size_t,
                ctypes.c_wchar_p,
                ctypes.c_wchar_p,
            )
            ...
            self.dll.BitmapGetPixel.argtypes = (
                ctypes.c_size_t,
                ctypes.c_int,
                ctypes.c_int,
            )
            self.dll.BitmapSetPixel.argtypes = (
                ctypes.c_size_t,
                ctypes.c_int,
                ctypes.c_int,
                ctypes.c_uint,
            ...
            self.dll = None
            Logger.WriteLine(
                "Can not load dll.\nFunctionalities related to Bitmap are not available.\nYou may need to install Microsoft Visual C++ 2015 Redistributable Package.",
                ConsoleColor.Yellow,
            )
...
GlobalKeyNames = ["CONTROL", "ALT", "SHIFT", "WIN", "CTRL", "LWIN", "RWIN"]
...
def SetClipboardText(text: str) -> bool:
    ...
    if ctypes.windll.user32.OpenClipboard(0):
        ...
        ctypes.windll.user32.SetClipboardData(
            ctypes.c_uint(13), ctypes.c_void_p(hClipboardData)
        )  # CF_TEXT=1, CF_UNICODETEXT=13
        ctypes.windll.user32.CloseClipboard()
        return True
    return False
...
def SetDpiAwareness(dpiAwarenessPerMonitor: bool = True) -> int:
    ...
    try:
        ...
        context = (
            DpiAwarenessContext.DpiAwarenessContextPerMonitorAware
            if dpiAwarenessPerMonitor
            else DpiAwarenessContext.DpiAwarenessContextUnaware
        )
        ...
        try:
            ...
            # Windows 8.1+
            if dpiAwarenessPerMonitor:
                return ctypes.windll.shcore.SetProcessDpiAwareness(
...
def GetMonitorsRect(dpiAwarenessPerMonitor: bool = False) -> List[Rect]:
    ...
    ret = ctypes.windll.user32.EnumDisplayMonitors(
...
def ShowWindow(handle: int, cmdShow: int) -> bool:
    ...
    return ctypes.windll.user32.ShowWindow(
        ctypes.c_void_p(handle), ctypes.c_int(cmdShow)
    )
...
def SetWindowTopmost(handle: int, isTopmost: bool) -> bool:
    ...
    return bool(
        SetWindowPos(handle, topValue, 0, 0, 0, 0, SWP.SWP_NoSize | SWP.SWP_NoMove)
    )
...
def IsProcess64Bit(processId: int) -> bool:
    ...
    try:
        func = (
            ctypes.windll.ntdll.ZwWow64ReadVirtualMemory64
        )  # only 64 bit OS has this function
    except Exception as ex:
        return False
    try:
            ...
            for j in range(count):
                if hold:
                    if upperKey in SpecialKeyNames:
                        ...
                        if (
                            type(lastKeyValue) == type(keyValue)
...
class Bitmap:
    ...
    def FromHandle(
        self, hwnd: int, left: int = 0, top: int = 0, right: int = 0, bottom: int = 0
    ) -> bool:
        """
        ...
        self._bitmap = _DllClient.instance().dll.BitmapFromWindow(
            root.NativeWindowHandle, left, top, right, bottom
        )
        self._getsize()
    ...
    def FromFile(self, filePath: str) -> bool:
        ...
        """
        self.Release()
        self._bitmap = _DllClient.instance().dll.BitmapFromFile(
    ...
    def ToFile(self, savePath: str) -> bool:
        ...
        return bool(
            _DllClient.instance().dll.BitmapToFile(
                self._bitmap,
                ctypes.c_wchar_p(savePath),
                ctypes.c_wchar_p(gdiplusImageFormat),
            )
        )
    ...
    def GetPixelColorsHorizontally(self, x: int, y: int, count: int) -> ctypes.Array:
        ...
        _DllClient.instance().dll.BitmapGetPixelsHorizontally(
            ctypes.c_size_t(self._bitmap), x, y, values, count
        )
        return values
    ...
    def SetPixelColorsHorizontally(self, x: int, y: int, colors: Iterable[int]) -> bool:
        ...
        return _DllClient.instance().dll.BitmapSetPixelsHorizontally(
            ctypes.c_size_t(self._bitmap), x, y, values, count
        )
    ...
    def GetPixelColorsVertically(self, x: int, y: int, count: int) -> ctypes.Array:
        ...
        arrayType = ctypes.c_uint32 * count
        values = arrayType()
        _DllClient.instance().dll.BitmapGetPixelsVertically(
            ctypes.c_size_t(self._bitmap), x, y, values, count
        )
        return values
    ...
    def SetPixelColorsVertically(self, x: int, y: int, colors: Iterable[int]) -> bool:
        ...
        arrayType = ctypes.c_uint32 * count
        values = arrayType(*colors)
        return _DllClient.instance().dll.BitmapSetPixelsVertically(
            ctypes.c_size_t(self._bitmap), x, y, values, count
        )
    ...
    def GetPixelColorsOfRect(
        ...
        values = arrayType()
        _DllClient.instance().dll.BitmapGetPixelsOfRect(
            ctypes.c_size_t(self._bitmap), x, y, width, height, values
        )
        return values

    def SetPixelColorsOfRect(
        self, x: int, y: int, width: int, height: int, colors: Iterable[int]
    ) -> bool:
        """
        x: int.
        y: int.
        ...
        values = arrayType(*colors)
        return bool(
            _DllClient.instance().dll.BitmapSetPixelsOfRect(
                ctypes.c_size_t(self._bitmap), x, y, width, height, values
            )
        )

    def GetPixelColorsOfRects(
        ...
        for x, y, w, h in rects:
            ...
            for row in range(h):
                colors.extend(
                    allColors[(y + row) * width + x : (y + row) * width + x + w]
                )
...
class Control:
    ...
    def __init__(
        self,
        searchFromControl: "Control" = None,
    ...
    def FindAll(
        self,
        find_mode: Literal[
            "All", "Ancestors", "Descendants", "Children", "Subtree", "Parent"
        ] = "All",
        ...
        if control_type:
            condition = (
                _AutomationClient.instance().IUIAutomation.CreatePropertyCondition(
                    PropertyId.ControlTypeProperty, getattr(ControlType, control_type)
                )
            )
        else:
    ...
    def GetChildControl(self, index: int, control_type: str = None) -> "Control":
        ...
        if control_type:
            children = [
                child for child in children if child.ControlTypeName == control_type
            ]
    ...
    def GetProgenyControl(
        self, depth: int = 1, index: int = 0, control_type: str = None, refresh=False
    ) -> "Control":
        """
        ...
        try:
            ...
            if control_type:
                controls = [
                    child for child in controls if child.ControlTypeName == control_type
                ]
    ...
    def Refind(
        ...
        if not self.Exists(
            ...
            if raiseException:
                ...
                raise LookupError(
                    "Find Control Timeout: " + self.GetSearchPropertiesStr()
                )
            else:
                return False
        return True
    ...
    def Click(
        ...
        if not hasattr(self, "winapi"):
            self.winapi = Win32(self.GetTopLevelControl().NativeWindowHandle)
        self.winapi.click_by_bbox(
            self.BoundingRectangle,
            xbias=x,
            ybias=y,
            pos=pos,
            activate=show_window,
            move=move,
        )
    ...
    def DoubleClick(
        ...
        if move:
            ...
            if not hasattr(self, "winapi"):
                self.winapi = Win32(self.GetTopLevelControl().NativeWindowHandle)
            self.winapi.click_by_bbox(
                self.BoundingRectangle, double_click=True, activate=show_window
            )
...
def WalkTree(
    top,
    getChildren: Callable[[TreeNode], List[TreeNode]] = None,
    getFirstChild: Callable[[TreeNode], TreeNode] = None,
    getNextSibling: Callable[[TreeNode], TreeNode] = None,
    yieldCondition: Callable[[TreeNode, int], bool] = None,
...
def LogControl(
    ...
    for pt, name in supportedPatterns:
        if isinstance(pt, ValuePattern):
            ...
            Logger.Write(
                "ToggleState." + getKeyName(ToggleState.__dict__, pt.ToggleState),
                ConsoleColor.DarkGreen,
            )
            ...
            Logger.Write(
                "ExpandCollapseState."
                + getKeyName(ExpandCollapseState.__dict__, pt.ExpandCollapseState),
                ConsoleColor.DarkGreen,
            )
...
if __name__ == "__main__":
    ...
    for i in sorted(
        [
            it
            for it in dir(_AutomationClient.instance().UIAutomationCore)
    ...
    for i in sorted(
        [
            it
            for it in dir(_AutomationClient.instance().IUIAutomation)
            if not it.startswith("_")
        ]
    ):
        print(i)
    ...
    for i in sorted(
        [
            it
            for it in dir(_AutomationClient.instance().ViewWalker)
            if not it.startswith("_")
        ]
    ):
        print(i)
