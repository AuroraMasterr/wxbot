...
class _ShapeBase:
    ...
    def set_color(self, color: str):
        self._ov._call(
            lambda: self._ov._canvas.itemconfig(self._id, outline=color, fill=color)
        )
...
class Rect(_ShapeBase):
    ...
    def move_to(self, x1: float, y1: float, x2: float, y2: float):
        self._ov._call(
            lambda: self._ov._canvas.coords(
                self._id, *self._ov._rect_coords(x1, y1, x2, y2)
            )
        )
...
class Circle(_ShapeBase):
    ...
    def __init__(
        self, overlay: "Overlay", item_id: int, cx: float, cy: float, r: float
    ):
...
class Overlay:
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=self._tk_thread_main, name="OverlayTk", daemon=True
        )
        self._thread.start()
        # 等到 Tk 初始化完成
    ...
    def add_rect(self, x1, y1, x2, y2, *, outline="red", width=4, fill=""):
        def _create():
            ...
            iid = self._canvas.create_rectangle(
                x1c, y1c, x2c, y2c, outline=outline, width=width, fill=fill
            )
    ...
    def add_circle(self, cx, cy, r, *, outline="lime", width=6, fill=""):
        def _create():
            ...
            iid = self._canvas.create_oval(
                x1, y1, x2, y2, outline=outline, width=width, fill=fill
            )
        ...
        if move:
            ...
            if button.lower() == "left":
                win32api.SendMessage(
                    ...
                    win32con.MK_LBUTTON,
                    win32api.MAKELONG(x1, y1),
                )
                win32api.SendMessage(
                    self.hwnd, win32con.WM_LBUTTONUP, 0, win32api.MAKELONG(x1, y1)
                )
            elif button.lower() == "right":
                win32api.SendMessage(
                    self.hwnd,
                ...
                win32api.SendMessage(
                    self.hwnd, win32con.WM_RBUTTONUP, 0, win32api.MAKELONG(x1, y1)
                )
            elif button.lower() == "mid":
                win32api.SendMessage(
                    self.hwnd,
                ...
                win32api.SendMessage(
                    self.hwnd, win32con.WM_MBUTTONUP, 0, win32api.MAKELONG(x1, y1)
                )
            else:
                raise ValueError("Invalid button")
...
def RollIntoView(win, ele, equal=True, bias=0):
    ...
    if attempt >= max_attempts:
        print(
            f"Warning: 滚动操作达到最大尝试次数({max_attempts})，可能元素无法完全滚动到视图内"
        )
...
def CheckElementPosition(win, ele, bias=0):
    ...
    result = {
        ...
        "ele_vertically_inside_win": win_top <= ele_top
        and ele_bottom <= win_bottom,  # ele垂直方向完全在win内
        "win_vertically_inside_ele": ele_top <= win_top
        and win_bottom <= ele_bottom,  # win垂直方向完全在ele内
        # 水平方向的关系
        ...
        "ele_horizontally_inside_win": win_left <= ele_left
        and ele_right <= win_right,  # ele水平方向完全在win内
        "win_horizontally_inside_ele": ele_left <= win_left
        and win_right <= ele_right,  # win水平方向完全在ele内
        # 综合关系
    ...
    vertical_overlap = not (
        result["ele_completely_above_win"] or result["ele_completely_below_win"]
    )
    horizontal_overlap = not (
        result["ele_completely_left_of_win"] or result["ele_completely_right_of_win"]
    )
