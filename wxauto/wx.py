...
from .ui.component import NewFriendElement, WeChatDialog
...
class Listener(ABC):
    def _listener_start(self):
        ...
        self._listener_thread = threading.Thread(
            target=self._listener_listen, daemon=True
        )
...
class Chat:
    ...
    @uilock
    def SendTypingText(self, msg, who=None, clear=True, exact=False) -> WxResponse:
    ...
    @uilock
    def SendFiles(self, filepath, who=None, exact=False) -> WxResponse:
        """向当前聊天窗口发送文件
    ...
    @uilock
    def SendEmotion(self, emotion_index, who=None, exact=False) -> WxResponse:
        """发送自定义表情
    ...
    def RemoveGroupMembers(
        self, group: str = None, members: Union[str, List[str]] = None
    ) -> WxResponse:
        """移除群成员
    ...
    def AddFriendFromGroup(
        ...
        Returns:
            WxResponse: 是否添加成功
        """
        return self._api.add_friend_from_group(
            index, who, addmsg, remark, tags, permission, exact
        )
    ...
    def ManageGroup(
        ...
        return self._api.manage_group(
            name=name, remark=remark, myname=myname, notice=notice, quit=quit
        )
...
class WeChat(Chat, Listener):
    ...
    def __init__(self, nickname: str = None, debug: bool = False, **kwargs):
        hwnd = None
        if "hwnd" in kwargs:
            hwnd = kwargs["hwnd"]
    ...
    def _get_listen_messages(self):
        ...
        for who in temp_listen:
            ...
            with self._lock:
                ...
                for msg in msgs:
                    wxlog.debug(
                        f"[{msg.attr} {msg.type}]获取到新消息：{who} - {msg.content}"
                    )
    ...
    def AddNewFriend(
        ...
        Returns:
            WxResponse: 添加好友的结果
        """
        return self._api.add_new_friend(
            keywords, addmsg, remark, tags, permission, timeout
        )
    ...
    def GetContactGroups(self, speed: int = 1, interval: float = 0.1) -> List[str]:
        """获取通讯录中的所有群聊
    ...
    def GetFriendDetails(
        ...
        """
        return self._api.get_friends_details(
            n, tag, timeout, save_head_image, save_head_wait
        )
...
def get_wx_clients() -> List[WeChat]:
    ...
    return [
        WeChat(hwnd=win[0]) for win in win32wins if win[1] == WeChatMainWnd._ui_cls_name
    ]
